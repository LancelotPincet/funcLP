import numpy as np
import numba as nb
from ._Polynomial3_cpukernel_function import _Polynomial3_cpukernel_function as model_scalar
...
def _Polynomial3_MLE_Binomial_cpu_assembly(
    ...
    for model in nb.prange(nmodels):
        ...
        for point in range(npoints):
            ...
            mod = model_scalar(point_x, model_a, model_b, model_c, model_d)
            ...
            count = 0
            if bool2fit[0]:
                jacob_local[count] = d_a(point_x, model_a, model_b, model_c, model_d)
                count += 1
            if bool2fit[1]:
                jacob_local[count] = d_b(point_x, model_a, model_b, model_c, model_d)
                count += 1
            if bool2fit[2]:
                jacob_local[count] = d_c(point_x, model_a, model_b, model_c, model_d)
                count += 1
            if bool2fit[3]:
                jacob_local[count] = d_d(point_x, model_a, model_b, model_c, model_d)
                count += 1
