...
from ._Polynomial2_gpukernel_function import _Polynomial2_gpukernel_function as model_scalar
...
def _Polynomial2_MLE_Binomial_gpu_assembly(
    ...
    for point in range(tid, npoints, bdim):
        ...
        mod = model_scalar(thread_x, block_a, block_b, block_c)
        ...
        if bool2fit[0]:
            jacob_local[count] = d_a(thread_x, block_a, block_b, block_c)
            count += 1

        if bool2fit[1]:
            jacob_local[count] = d_b(thread_x, block_a, block_b, block_c)
            count += 1

        if bool2fit[2]:
            jacob_local[count] = d_c(thread_x, block_a, block_b, block_c)
            count += 1
