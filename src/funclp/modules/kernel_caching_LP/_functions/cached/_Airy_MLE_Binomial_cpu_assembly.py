import numpy as np
import numba as nb
from ._Airy_cpukernel_function import _Airy_cpukernel_function as model_scalar
...
def _Airy_MLE_Binomial_cpu_assembly(
    ...
    for model in nb.prange(nmodels):
        ...
        for point in range(npoints):
            ...
            mod = model_scalar(point_x, model_mu, model_amp, model_offset, model_wl, model_NA, model_tol)
            ...
            count = 0
            if bool2fit[0]:
                jacob_local[count] = d_mu(point_x, model_mu, model_amp, model_offset, model_wl, model_NA, model_tol)
                count += 1
            if bool2fit[1]:
                jacob_local[count] = d_amp(point_x, model_mu, model_amp, model_offset, model_wl, model_NA, model_tol)
                count += 1
            if bool2fit[2]:
                jacob_local[count] = d_offset(point_x, model_mu, model_amp, model_offset, model_wl, model_NA, model_tol)
                count += 1
            if bool2fit[3]:
                jacob_local[count] = d_wl(point_x, model_mu, model_amp, model_offset, model_wl, model_NA, model_tol)
                count += 1
            if bool2fit[4]:
                jacob_local[count] = d_NA(point_x, model_mu, model_amp, model_offset, model_wl, model_NA, model_tol)
                count += 1
            if bool2fit[5]:
                jacob_local[count] = d_tol(point_x, model_mu, model_amp, model_offset, model_wl, model_NA, model_tol)
                count += 1
