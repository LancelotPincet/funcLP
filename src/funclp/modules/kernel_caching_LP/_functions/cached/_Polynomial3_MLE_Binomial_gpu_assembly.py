...
from ._Polynomial3_gpukernel_function import _Polynomial3_gpukernel_function as model_scalar
...
def _Polynomial3_MLE_Binomial_gpu_assembly(
    raw_data, x, a, b, c, d, weights, chi2, gradient, hessian, bool2fit, ignore
):
    ...
    for point in range(tid, npoints, bdim):
        ...
        mod = model_scalar(thread_x, block_a, block_b, block_c, block_d)
        ...
        if bool2fit[0]:
            jacob_local[count] = d_a(thread_x, block_a, block_b, block_c, block_d)
            count += 1

        if bool2fit[1]:
            jacob_local[count] = d_b(thread_x, block_a, block_b, block_c, block_d)
            count += 1

        if bool2fit[2]:
            jacob_local[count] = d_c(thread_x, block_a, block_b, block_c, block_d)
            count += 1

        if bool2fit[3]:
            jacob_local[count] = d_d(thread_x, block_a, block_b, block_c, block_d)
            count += 1
