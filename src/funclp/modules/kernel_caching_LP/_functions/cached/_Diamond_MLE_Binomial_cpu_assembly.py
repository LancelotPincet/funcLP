...
def _Diamond_MLE_Binomial_cpu_assembly(
    ...
    for model in nb.prange(nmodels):
        ...
        for point in range(npoints):
            ...
            count = 0
            if bool2fit[0]:
                jacob_local[count] = d_d(point_x, point_y, model_d, model_mux, model_muy, model_amp, model_offset)
                count += 1
            if bool2fit[1]:
                jacob_local[count] = d_mux(point_x, point_y, model_d, model_mux, model_muy, model_amp, model_offset)
                count += 1
            if bool2fit[2]:
                jacob_local[count] = d_muy(point_x, point_y, model_d, model_mux, model_muy, model_amp, model_offset)
                count += 1
            if bool2fit[3]:
                jacob_local[count] = d_amp(point_x, point_y, model_d, model_mux, model_muy, model_amp, model_offset)
                count += 1
            if bool2fit[4]:
                jacob_local[count] = d_offset(point_x, point_y, model_d, model_mux, model_muy, model_amp, model_offset)
                count += 1
