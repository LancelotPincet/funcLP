...
from ._Disc_gpukernel_function import _Disc_gpukernel_function as model_scalar
...
def _Disc_MLE_Binomial_gpu_assembly(
    ...
    for point in range(tid, npoints, bdim):
        ...
        mod = model_scalar(thread_x, thread_y, block_r, block_mux, block_muy, block_amp, block_offset)
        ...
        if bool2fit[0]:
            jacob_local[count] = d_r(thread_x, thread_y, block_r, block_mux, block_muy, block_amp, block_offset)
            count += 1

        if bool2fit[1]:
            jacob_local[count] = d_mux(thread_x, thread_y, block_r, block_mux, block_muy, block_amp, block_offset)
            count += 1

        if bool2fit[2]:
            jacob_local[count] = d_muy(thread_x, thread_y, block_r, block_mux, block_muy, block_amp, block_offset)
            count += 1

        if bool2fit[3]:
            jacob_local[count] = d_amp(thread_x, thread_y, block_r, block_mux, block_muy, block_amp, block_offset)
            count += 1

        if bool2fit[4]:
            jacob_local[count] = d_offset(thread_x, thread_y, block_r, block_mux, block_muy, block_amp, block_offset)
            count += 1
