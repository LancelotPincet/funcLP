import numpy as np
import numba as nb
from ._Rectangle_cpukernel_function import _Rectangle_cpukernel_function as model_scalar
...
def _Rectangle_MLE_Binomial_cpu_assembly(
    ...
    for model in nb.prange(nmodels):
        ...
        for point in range(npoints):
            ...
            mod = model_scalar(point_x, point_y, model_l, model_ratio, model_mux, model_muy, model_amp, model_offset, model_theta)
            ...
            count = 0
            if bool2fit[0]:
                jacob_local[count] = d_l(point_x, point_y, model_l, model_ratio, model_mux, model_muy, model_amp, model_offset, model_theta)
                count += 1
            if bool2fit[1]:
                jacob_local[count] = d_ratio(point_x, point_y, model_l, model_ratio, model_mux, model_muy, model_amp, model_offset, model_theta)
                count += 1
            if bool2fit[2]:
                jacob_local[count] = d_mux(point_x, point_y, model_l, model_ratio, model_mux, model_muy, model_amp, model_offset, model_theta)
                count += 1
            if bool2fit[3]:
                jacob_local[count] = d_muy(point_x, point_y, model_l, model_ratio, model_mux, model_muy, model_amp, model_offset, model_theta)
                count += 1
            if bool2fit[4]:
                jacob_local[count] = d_amp(point_x, point_y, model_l, model_ratio, model_mux, model_muy, model_amp, model_offset, model_theta)
                count += 1
            if bool2fit[5]:
                jacob_local[count] = d_offset(point_x, point_y, model_l, model_ratio, model_mux, model_muy, model_amp, model_offset, model_theta)
                count += 1
            if bool2fit[6]:
                jacob_local[count] = d_theta(point_x, point_y, model_l, model_ratio, model_mux, model_muy, model_amp, model_offset, model_theta)
                count += 1
