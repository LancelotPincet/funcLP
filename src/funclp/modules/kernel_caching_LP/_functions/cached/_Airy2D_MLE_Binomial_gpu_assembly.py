...
def _Airy2D_MLE_Binomial_gpu_assembly(
    ...
    for point in range(tid, npoints, bdim):
        ...
        mod = model_scalar(thread_x, thread_y, block_mux, block_muy, block_amp, block_offset, block_wl, block_NA, block_tol)
        ...
        if bool2fit[0]:
            jacob_local[count] = d_mux(thread_x, thread_y, block_mux, block_muy, block_amp, block_offset, block_wl, block_NA, block_tol)
            count += 1

        if bool2fit[1]:
            jacob_local[count] = d_muy(thread_x, thread_y, block_mux, block_muy, block_amp, block_offset, block_wl, block_NA, block_tol)
            count += 1

        if bool2fit[2]:
            jacob_local[count] = d_amp(thread_x, thread_y, block_mux, block_muy, block_amp, block_offset, block_wl, block_NA, block_tol)
            count += 1

        if bool2fit[3]:
            jacob_local[count] = d_offset(thread_x, thread_y, block_mux, block_muy, block_amp, block_offset, block_wl, block_NA, block_tol)
            count += 1

        if bool2fit[4]:
            jacob_local[count] = d_wl(thread_x, thread_y, block_mux, block_muy, block_amp, block_offset, block_wl, block_NA, block_tol)
            count += 1

        if bool2fit[5]:
            jacob_local[count] = d_NA(thread_x, thread_y, block_mux, block_muy, block_amp, block_offset, block_wl, block_NA, block_tol)
            count += 1

        if bool2fit[6]:
            jacob_local[count] = d_tol(thread_x, thread_y, block_mux, block_muy, block_amp, block_offset, block_wl, block_NA, block_tol)
            count += 1
