import numpy as np
import numba as nb
from ._Exponential1_cpukernel_function import _Exponential1_cpukernel_function as model_scalar
...
def _Exponential1_MLE_Binomial_cpu_assembly(
    ...
    for model in nb.prange(nmodels):
        ...
        for point in range(npoints):
            ...
            mod = model_scalar(point_t, model_tau, model_amp, model_offset)
            ...
            count = 0
            if bool2fit[0]:
                jacob_local[count] = d_tau(point_t, model_tau, model_amp, model_offset)
                count += 1
            if bool2fit[1]:
                jacob_local[count] = d_amp(point_t, model_tau, model_amp, model_offset)
                count += 1
            if bool2fit[2]:
                jacob_local[count] = d_offset(point_t, model_tau, model_amp, model_offset)
                count += 1
