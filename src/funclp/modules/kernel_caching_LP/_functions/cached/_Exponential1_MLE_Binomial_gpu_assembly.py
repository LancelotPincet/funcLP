...
from ._Exponential1_gpukernel_function import _Exponential1_gpukernel_function as model_scalar
...
def _Exponential1_MLE_Binomial_gpu_assembly(
    raw_data, t, tau, amp, offset, weights, chi2, gradient, hessian, bool2fit, ignore
):
    ...
    for point in range(tid, npoints, bdim):
        ...
        mod = model_scalar(thread_t, block_tau, block_amp, block_offset)
        ...
        if bool2fit[0]:
            jacob_local[count] = d_tau(thread_t, block_tau, block_amp, block_offset)
            count += 1

        if bool2fit[1]:
            jacob_local[count] = d_amp(thread_t, block_tau, block_amp, block_offset)
            count += 1

        if bool2fit[2]:
            jacob_local[count] = d_offset(thread_t, block_tau, block_amp, block_offset)
            count += 1
