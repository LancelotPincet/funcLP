...
def _Polynomial1_MLE_Binomial_gpu_assembly(
    raw_data, x, a, b, weights, chi2, gradient, hessian, bool2fit, ignore
):
    ...
    for point in range(tid, npoints, bdim):
        ...
        mod = model_scalar(thread_x, block_a, block_b)
        ...
        if bool2fit[0]:
            jacob_local[count] = d_a(thread_x, block_a, block_b)
            count += 1

        if bool2fit[1]:
            jacob_local[count] = d_b(thread_x, block_a, block_b)
            count += 1
