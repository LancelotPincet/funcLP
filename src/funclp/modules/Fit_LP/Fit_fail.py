...
class Fit(ABC, CudaReference) :
    ...
    def __call__(self, raw_data, *args, weights=np.float32(1.)) :
        ...
        self.parameter_scale = self.xp.empty(shape=(self.nmodels, self.nparameters2fit), dtype=self.dtype) # Step to apply on each parameter
    ...
    def jacobian(self) :
        ...
        jacobian_function(*self.variables, *self.data, *self.parameters, self.jacobian_data, self.converged)
    ...
    def cpu_jacobian(self) :
        ...
        inputs = ', '.join(self.function.variables + self.function.data + list(self.function.parameters.keys()) + list(self.function.constants.keys()))
        indexes = ', '.join([f'{key}[point]' for key in self.function.variables] + [f'{key}[model, point]' for key in self.function.data] + [f'{key}[model]' for key in self.function.parameters] + [key for key in self.function.constants])
        string = f'''
@nb.njit(parallel=True)
...
'''
        for param, parameter in enumerate(self.parameters2fit) :
            string += f'''
            jacobian[model, point, {param}] = d_{parameter}({indexes})
'''
        glob = {'nb': nb}
        glob.update(kernels)
        loc = {}
