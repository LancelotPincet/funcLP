...
import math
@ufunc(data=[], constants=['tx', 'ty', 'coeffs'], fastmath=False)
def d_mux(x, y, /, mux, muy, amp, offset, kx, ky, tx, ty, coeffs):
    eps = 1e-3 * max(1.0, abs(mux))
    f_plus = kernel(x, y, mux + eps, muy, amp, offset, kx, ky, tx, ty, coeffs)
    f_minus = kernel(x, y, mux - eps, muy, amp, offset, kx, ky, tx, ty, coeffs)
