...
def d_kx(x, y, /, mux, muy, amp, offset, kx, ky, tx, ty, coeffs):
    eps = 1e-3 * max(1.0, abs(kx))
    f_plus = kernel(x, y, mux, muy, amp, offset, kx + eps, ky, tx, ty, coeffs)
    f_minus = kernel(x, y, mux, muy, amp, offset, kx - eps, ky, tx, ty, coeffs)
