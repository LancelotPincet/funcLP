...
def d_pixy(x, y, /, mux, muy, sigx, sigy, amp, offset, pixx, pixy, nsig, theta):
    eps = 1e-3 * max(1.0, abs(pixy))
    f_plus = kernel(x, y, mux, muy, sigx, sigy, amp, offset, pixx, pixy + eps, nsig, theta)
    f_minus = kernel(x, y, mux, muy, sigx, sigy, amp, offset, pixx, pixy - eps, nsig, theta)
