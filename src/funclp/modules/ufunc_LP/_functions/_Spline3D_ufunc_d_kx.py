...
def d_kx(x, y, z, /, mux, muy, muz, amp, offset, kx, ky, kz, tx, ty, tz, coeffs):
    eps = 1e-3 * max(1.0, abs(kx))
    f_plus = kernel(x, y, z, mux, muy, muz, amp, offset, kx + eps, ky, kz, tx, ty, tz, coeffs)
    f_minus = kernel(x, y, z, mux, muy, muz, amp, offset, kx - eps, ky, kz, tx, ty, tz, coeffs)
