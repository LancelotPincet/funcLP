...
def d_muy(x, y, /, mux, muy, amp, offset, kx, ky, tx, ty, coeffs):
    eps = 1e-3 * max(1.0, abs(muy))
    f_plus = kernel(x, y, mux, muy + eps, amp, offset, kx, ky, tx, ty, coeffs)
    f_minus = kernel(x, y, mux, muy - eps, amp, offset, kx, ky, tx, ty, coeffs)
