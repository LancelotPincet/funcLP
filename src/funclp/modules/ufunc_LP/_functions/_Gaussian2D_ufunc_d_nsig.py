...
def d_nsig(x, y, /, mux, muy, sigx, sigy, amp, offset, pixx, pixy, nsig, theta):
    eps = 1e-3 * max(1.0, abs(nsig))
    f_plus = kernel(x, y, mux, muy, sigx, sigy, amp, offset, pixx, pixy, nsig + eps, theta)
    f_minus = kernel(x, y, mux, muy, sigx, sigy, amp, offset, pixx, pixy, nsig - eps, theta)
