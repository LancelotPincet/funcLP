
from ._IsoGaussian_cpukernel_function import _IsoGaussian_cpukernel_function as kernel
from funclp import Parameter, ufunc
import math
@ufunc(variables=['x', 'y'], data=[], parameters=[Parameter('mux', 0.0), Parameter('muy', 0.0), Parameter('sig', 0.15915494309189535), Parameter('amp', 1.0), Parameter('offset', 0.0), Parameter('pixx', -1.0), Parameter('pixy', -1.0), Parameter('nsig', -1.0)], constants=[], fastmath=False)
def d_pixx(x, y, /, mux, muy, sig, amp, offset, pixx, pixy, nsig):
    eps = 1e-3 * max(1.0, abs(pixx))
    f_plus = kernel(x, y, mux, muy, sig, amp, offset, pixx + eps, pixy, nsig)
    f_minus = kernel(x, y, mux, muy, sig, amp, offset, pixx - eps, pixy, nsig)
    if math.isfinite(f_plus) and math.isfinite(f_minus):
        return (f_plus - f_minus) / (2.0 * eps)
    f_x = kernel(x, y, mux, muy, sig, amp, offset, pixx, pixy, nsig)
    if math.isfinite(f_plus) and math.isfinite(f_x):
        return (f_plus - f_x) / eps
    if math.isfinite(f_minus) and math.isfinite(f_x):
        return (f_x - f_minus) / eps
    return math.nan
