...
def d_amp(x, y, /, l, ratio, mux, muy, amp, offset, theta):
    eps = 1e-3 * max(1.0, abs(amp))
    f_plus = kernel(x, y, l, ratio, mux, muy, amp + eps, offset, theta)
    f_minus = kernel(x, y, l, ratio, mux, muy, amp - eps, offset, theta)
