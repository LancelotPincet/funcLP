...
def d_amp(x, y, /, mux, muy, amp, offset, kx, ky, tx, ty, coeffs):
    eps = 1e-3 * max(1.0, abs(amp))
    f_plus = kernel(x, y, mux, muy, amp + eps, offset, kx, ky, tx, ty, coeffs)
    f_minus = kernel(x, y, mux, muy, amp - eps, offset, kx, ky, tx, ty, coeffs)
